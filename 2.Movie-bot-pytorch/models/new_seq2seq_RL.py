...
class NewSeq2seqRL(NewSeq2seq):
    ...
    def forward(self, inputs, targets, rewards):
        ...
        decoder_outputs = Variable(torch.zeros(self.time_steps, b, self.num_tokens))  # (time_steps, batch_size, vocab_size)
        ...
        for t in range(self.time_steps):
            outputs1, hidden1 = self.lstm1(torch.zeros(1, b, self.latent_dim).to(self.device), hidden1)
            outputs2, hidden2 = self.lstm2(torch.cat([ground_truth_embedded[t].unsqueeze(0),outputs1],-1),hidden2) # (1, batch_size, latent_dim)
            ...
            entropies.append(pg_current_loss)
        
        return decoder_outputs, pg_loss / opt.mxlen, entropies
    ...
    def evaluation(self, inputs):
        ...
        # Define initial input
        decoder_input = Variable(torch.LongTensor([[self.sos_id]*batch_size])).to(self.device)
        decoder_input_embedded = self.embedding(decoder_input)
        decoder_input_embedded = self.dense(decoder_input_embedded)
        
        # Initialize output container
        decoder_outputs = Variable(torch.zeros(self.time_steps, batch_size, self.num_tokens)).to(self.device)  # (time_steps, batch_size, vocab_size)
        decoded_indices = Variable(torch.zeros(self.time_steps, batch_size, dtype=torch.int32).long()).to(self.device) # (time_steps, batch_size)

        # Unfold the decoder RNN on the time dimension
        
        for t in range(self.time_steps):
            outputs1, hidden1 = self.lstm1(torch.zeros(1, batch_size, self.latent_dim).to(self.device), hidden1)
            outputs2, hidden2 = self.lstm2(torch.cat([decoder_input_embedded,outputs1],-1),hidden2)
            outputs2 = self.log_softmax(self.out(outputs2))
            decoder_outputs[t] = outputs2
