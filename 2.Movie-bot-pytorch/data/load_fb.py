...
class FbData:
    ...
    def load_conversations(self):
        """
            Load the fb message file "only_messages.txt".
            The ﬁle has single ----- which represents the conversation split
            -----
            反正不要在Berkeley xd
            真的 我要以英鎊計價
            ...
        ...
        for line in file:
            ...
            if line.find("\n") != -1:
                line = line[:-1]
            words = jieba.cut(line, cut_all=False)
            cur = ["<START>"]+list(map(func,words))[:self.mxlen-2]+["<EOS>"]
            
            # No prev sentence, pass
            if prev == []:
                prev = cur
            else:
                all_convs.append((cur, prev))
                num_data += 1
                prev = cur
...
if __name__ == '__main__':
    ...
    data = FbData("only_messages.txt", "fb_data.bin", False)
